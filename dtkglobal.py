...
def SaveSettings(advSettingIn, unlockSettingIn, unzipSettingIn, defaultPackagesToExcludeIn, defaultMetadataFolderIn, defaultScriptFolderIn, defaultApiVersionIn):
            ...
            for key in dictSource:
                if key in dictTarget:
                    ...
                    if dictSource[key] in allTextToReplaceContent:
                        ...
                        allTextToReplaceContentFile.close()
            if (replaced):
                returnMsg = "Replaced values from file " + pathStringFileToReplaceContent + " using " + pathStringFileSource + " as source and " + pathStringFileTarget + " as target. Match fields: " + matchFieldGrand + ". Replace: " + replaceField + "."
    else:
        error = True
...
def ProcessDataScriptLine(lineSplit, lineStr, targetName, sourceName, deployDataUrl, lineNumber, deployStageUrl):
    ...
    if len(lineSplit) >= 3:
        if lineSplit[1] == "SOQLQUERY":
            ...
            cmd = 'sfdx force:data:soql:query --apiversion ' + defaultApiVersion + ' -u ' + target + ' -q ' + query + ' -r csv'
        if lineSplit[1] == "BULKDELETE":
            ...
            if not os.path.exists(pathString):
                ...
                return error, errorMsg, target, pathString, cmd
            cmd = ['sfdx', 'force:data:bulk:delete', '--apiversion', defaultApiVersion, '-u', target, '-s', customObject, '-f', pathString]
        if lineSplit[1] == "BULKUPSERT":
            ...
            if not os.path.exists(pathString):
                ...
                return error, errorMsg, target, pathString, cmd
            cmd = ['sfdx', 'force:data:bulk:upsert', '--apiversion', defaultApiVersion, '-u', target, '-s', customObject, '-i', externalId, '-f', pathString]
        if lineSplit[1] == "RECORDCREATE":
            ...
            customObject = lineSplit[2]
            values = lineSplit[3];
            cmd = ['sfdx', 'force:data:record:create', '--apiversion', defaultApiVersion, '-u', target, '-s', customObject, '-q', '\"' + values + '\"']
            cmd = 'sfdx force:data:record:create --apiversion ' + defaultApiVersion + ' -u ' + target + ' -s ' + customObject + ' -q ' + values
        if lineSplit[1] == "RECORDDELETE":
            customObject = lineSplit[2]
            values = lineSplit[3];
            externalId = lineSplit[4]
            cmd = 'sfdx force:data:record:delete --apiversion ' + defaultApiVersion + ' -u ' + target + ' -s ' + customObject + ' -i ' + values
            if externalId == "values":
                cmd = 'sfdx force:data:record:delete --apiversion ' + defaultApiVersion + ' -u ' + target + ' -s ' + customObject + ' -w ' + values
        if lineSplit[1] == "RECORDUPDATE":
            ...
            cmd = 'sfdx force:data:record:update --apiversion ' + defaultApiVersion + ' -u ' + target + ' -s ' + customObject + ' -i ' + values + ' -v ' + valuesToUpdate
            if externalId == "values":
                cmd = 'sfdx force:data:record:update --apiversion ' + defaultApiVersion + ' -u ' + target + ' -s ' + customObject + ' -w ' + values + ' -v ' + valuesToUpdate
